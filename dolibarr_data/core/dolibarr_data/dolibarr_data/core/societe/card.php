...
if (is_object($objcanvas) && $objcanvas->displayCanvasExists($canvasdisplayaction)) {
	...
	// Ajouter l'onglet Factures après l'onglet Customer
	if (isModEnabled("facture") && ($object->client == 1 || $object->client == 3) && $user->hasRight("facture", "lire")) {
		// Trouver l'index de l'onglet "customer"
		$insertIndex = -1;
		foreach ($head as $index => $tab) {
			if (isset($tab[2]) && $tab[2] == "customer") {
				$insertIndex = $index + 1;
				break;
			}
		}
		if ($insertIndex >= 0) {
			...
			$newTab = array();
			$newTab[0] = DOL_URL_ROOT."/custom/societe/invoices.php?socid=".$object->id;
			$newTab[1] = $langs->trans("Invoices");
			if ($nbFactures > 0) {
				$newTab[1] .= "<span class=\"badge marginleftonlyshort\">".$nbFactures."</span>";
			}
			$newTab[2] = "invoices";
			array_splice($head, $insertIndex, 0, array($newTab));
		}
	}
		...
		if ($action != 'presend') {
			...
			if (empty($reshook)) {
				...
				// Bouton Créer une proposition commerciale
				if (isModEnabled("propal") && ($object->client == 1 || $object->client == 3) && $user->hasRight("propal", "creer")) {
					print dolGetButtonAction("", "Nouvelle proposition", "default", DOL_URL_ROOT."/comm/propal/card.php?action=create&socid=".$object->id, "", 1);
				}
