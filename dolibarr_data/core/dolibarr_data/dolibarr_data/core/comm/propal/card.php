...
// 		// Project
		if (isModEnabled('project') && is_object($formproject)) {
			...
			print '<style>.field_projectid select { opacity: 0.6; pointer-events: none; background-color: #f5f5f5; cursor: not-allowed; }</style>';
			print '<script>document.addEventListener("DOMContentLoaded", function() { var select = document.querySelector(".field_projectid select"); if(select) select.disabled = true; });</script>';
